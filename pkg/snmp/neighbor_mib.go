...
func (a *Agent) initializeIFMIB() {
	...
	if len(device.TrunkPorts) == 0 {
		...
		a.createInterfaceEntry(1, "Management", device.MACAddress.String(), 1000000000)
	} else {
...
func (a *Agent) createInterfaceEntry(ifIdx int, interfaceName, mac string, speedBps uint64) {
	...
	// ifIndex
	a.mib.Set(ifIndex+"."+idxStr, &OIDValue{
		Type:  gosnmp.Integer,
		Value: ifIdx,
	})

	// ifDescr
	a.mib.Set(ifDescr+"."+idxStr, &OIDValue{
		Type:  gosnmp.OctetString,
		Value: interfaceName,
	})

	// ifType (6 = ethernetCsmacd)
	a.mib.Set(ifType+"."+idxStr, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 6,
	})

	// ifMtu
	a.mib.Set(ifMtu+"."+idxStr, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 1500,
	})
	...
	a.mib.Set(ifPhysAddress+"."+idxStr, &OIDValue{
		Type:  gosnmp.OctetString,
		Value: macBytes,
	})

	// ifAdminStatus (1 = up)
	a.mib.Set(ifAdminStatus+"."+idxStr, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 1,
	})

	// ifOperStatus (1 = up)
	a.mib.Set(ifOperStatus+"."+idxStr, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 1,
	})
	...
	// Interface counters - dynamic to simulate traffic
	startTime := a.startTime

	// ifInOctets (input bytes - simulate ~1MB/s base traffic per interface)
	...
	// ifInNUcastPkts (broadcast/multicast)
	...
	// ifInDiscards (very low)
	a.mib.Set(ifInDiscards+"."+idxStr, &OIDValue{Type: gosnmp.Counter32, Value: uint32(0)})

	// ifInErrors (very low)
	a.mib.Set(ifInErrors+"."+idxStr, &OIDValue{Type: gosnmp.Counter32, Value: uint32(0)})

	// ifInUnknownProtos
	a.mib.Set(ifInUnknownProtos+"."+idxStr, &OIDValue{Type: gosnmp.Counter32, Value: uint32(0)})
	...
	// ifOutDiscards
	a.mib.Set(ifOutDiscards+"."+idxStr, &OIDValue{Type: gosnmp.Counter32, Value: uint32(0)})

	// ifOutErrors
	a.mib.Set(ifOutErrors+"."+idxStr, &OIDValue{Type: gosnmp.Counter32, Value: uint32(0)})

	// ifOutQLen (output queue length)
	a.mib.Set(ifOutQLen+"."+idxStr, &OIDValue{Type: gosnmp.Gauge32, Value: uint32(0)})

	// ifSpecific (deprecated, but some tools expect it)
	a.mib.Set(ifSpecific+"."+idxStr, &OIDValue{
		Type:  gosnmp.ObjectIdentifier,
		Value: "0.0",
	})

	// =========== ifXTable entries (extended interface table) ===========

	// ifName - canonical interface name
	a.mib.Set(ifName+"."+idxStr, &OIDValue{
		Type:  gosnmp.OctetString,
		Value: interfaceName,
	})
	...
	a.mib.Set(ifLinkUpDownTrapEnable+"."+idxStr, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 1,
	})
	...
	a.mib.Set(ifPromiscuousMode+"."+idxStr, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 2,
	})

	// ifConnectorPresent (1 = true)
	a.mib.Set(ifConnectorPresent+"."+idxStr, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 1,
	})

	// ifAlias - interface description/alias
	a.mib.Set(ifAlias+"."+idxStr, &OIDValue{
		Type:  gosnmp.OctetString,
		Value: interfaceName,
	})

	// ifCounterDiscontinuityTime
	a.mib.Set(ifCounterDiscontinuityTime+"."+idxStr, &OIDValue{
		Type:  gosnmp.TimeTicks,
		Value: uint32(0),
	})
}
...
func (a *Agent) initializeIPMIB() {
	...
	// Create ipAddrTable entries for each IP address
	for idx, ip := range ips {
		...
		// ipAdEntAddr - the IP address itself (index)
		a.mib.Set(ipAdEntAddr+"."+ipStr, &OIDValue{
			Type:  gosnmp.IPAddress,
			Value: ipStr,
		})

		// ipAdEntIfIndex - interface index
		a.mib.Set(ipAdEntIfIndex+"."+ipStr, &OIDValue{
			Type:  gosnmp.Integer,
			Value: ifIndex,
		})

		// ipAdEntNetMask - subnet mask (assume /24 for simplicity)
		a.mib.Set(ipAdEntNetMask+"."+ipStr, &OIDValue{
			Type:  gosnmp.IPAddress,
			Value: "255.255.255.0",
		})

		// ipAdEntBcastAddr - broadcast address LSB (1 = host bits all ones)
		a.mib.Set(ipAdEntBcastAddr+"."+ipStr, &OIDValue{
			Type:  gosnmp.Integer,
			Value: 1,
		})

		// ipAdEntReasmMaxSize - max reassembly size
		a.mib.Set(ipAdEntReasmMaxSize+"."+ipStr, &OIDValue{
			Type:  gosnmp.Integer,
			Value: 65535,
		})
	}

	// Create a default route entry (0.0.0.0)
	a.mib.Set(ipRouteDest+".0.0.0.0", &OIDValue{
		Type:  gosnmp.IPAddress,
		Value: "0.0.0.0",
	})
	a.mib.Set(ipRouteIfIndex+".0.0.0.0", &OIDValue{
		Type:  gosnmp.Integer,
		Value: 1,
	})
	a.mib.Set(ipRouteMetric1+".0.0.0.0", &OIDValue{
		Type:  gosnmp.Integer,
		Value: 1,
	})
	// Default gateway - use first IP with last octet as .1
	if len(ips) > 0 && ips[0].To4() != nil {
		ip4 := ips[0].To4()
		gateway := fmt.Sprintf("%d.%d.%d.1", ip4[0], ip4[1], ip4[2])
		a.mib.Set(ipRouteNextHop+".0.0.0.0", &OIDValue{
			Type:  gosnmp.IPAddress,
			Value: gateway,
		})
	}
	...
	a.mib.Set(ipRouteProto+".0.0.0.0", &OIDValue{
		Type:  gosnmp.Integer,
		Value: 2, // local
	})
	a.mib.Set(ipRouteMask+".0.0.0.0", &OIDValue{
		Type:  gosnmp.IPAddress,
		Value: "0.0.0.0",
	})
	...
	for _, trunk := range device.TrunkPorts {
		...
		// Create a synthetic ARP entry for each neighbor
		// Index is: ifIndex.ipAddress
		ifIdx := arpIndex
		// Use a synthetic IP based on position (10.0.0.x)
		neighborIP := fmt.Sprintf("10.0.0.%d", arpIndex)
		indexStr := fmt.Sprintf("%d.%s", ifIdx, neighborIP)

		// ipNetToMediaIfIndex
		a.mib.Set(ipNetToMediaIfIndex+"."+indexStr, &OIDValue{
			Type:  gosnmp.Integer,
			Value: ifIdx,
		})

		// ipNetToMediaPhysAddress - synthetic MAC for neighbor
		macBytes := []byte{0x00, 0x00, 0x00, 0x00, 0x00, byte(arpIndex)}
		a.mib.Set(ipNetToMediaPhysAddress+"."+indexStr, &OIDValue{
			Type:  gosnmp.OctetString,
			Value: macBytes,
		})

		// ipNetToMediaNetAddress
		a.mib.Set(ipNetToMediaNetAddress+"."+indexStr, &OIDValue{
			Type:  gosnmp.IPAddress,
			Value: neighborIP,
		})

		// ipNetToMediaType (3 = dynamic)
		a.mib.Set(ipNetToMediaType+"."+indexStr, &OIDValue{
			Type:  gosnmp.Integer,
			Value: 3,
		})
...
func (a *Agent) initializeBridgeMIB() {
	...
	stpEnabled := device.STPConfig != nil && device.STPConfig.Enabled

	// dot1dBase group - always populate for network devices
	numPorts := len(device.TrunkPorts)
	if numPorts == 0 {
		numPorts = 1
	}

	// dot1dBaseBridgeAddress - bridge MAC address
	macBytes := parseMACBytes(device.MACAddress.String())
	a.mib.Set(dot1dBaseBridgeAddress, &OIDValue{
		Type:  gosnmp.OctetString,
		Value: macBytes,
	})

	// dot1dBaseNumPorts
	a.mib.Set(dot1dBaseNumPorts, &OIDValue{
		Type:  gosnmp.Integer,
		Value: numPorts,
	})

	// dot1dBaseType (2 = transparent-only bridging)
	a.mib.Set(dot1dBaseType, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 2,
	})
	...
	for portIdx := 1; portIdx <= numPorts; portIdx++ {
		...
		// dot1dBasePortIfIndex - map to IF-MIB interface
		a.mib.Set(dot1dBasePortIfIndex+"."+portStr, &OIDValue{
			Type:  gosnmp.Integer,
			Value: portIdx,
		})

		// dot1dBasePortCircuit
		a.mib.Set(dot1dBasePortCircuit+"."+portStr, &OIDValue{
			Type:  gosnmp.ObjectIdentifier,
			Value: "0.0",
		})

		// dot1dBasePortDelayExceededDiscards
		a.mib.Set(dot1dBasePortDelayExceededDiscards+"."+portStr, &OIDValue{
			Type:  gosnmp.Counter32,
			Value: uint32(0),
		})

		// dot1dBasePortMtuExceededDiscards
		a.mib.Set(dot1dBasePortMtuExceededDiscards+"."+portStr, &OIDValue{
			Type:  gosnmp.Counter32,
			Value: uint32(0),
		})
	}

	// dot1dStp group - STP configuration
	if stpEnabled {
		stp := device.STPConfig

		// dot1dStpProtocolSpecification (3 = ieee8021d)
		a.mib.Set(dot1dStpProtocolSpecification, &OIDValue{
			Type:  gosnmp.Integer,
			Value: 3,
		})

		// dot1dStpPriority
		priority := stp.BridgePriority
		if priority == 0 {
			priority = 32768
		}
		...
		// dot1dStpTopChanges
		a.mib.Set(dot1dStpTopChanges, &OIDValue{
			Type:  gosnmp.Counter32,
			Value: uint32(0),
		})

		// dot1dStpDesignatedRoot - BridgeID format (priority + MAC)
		bridgeID := make([]byte, 8)
		bridgeID[0] = byte(priority >> 8)
		bridgeID[1] = byte(priority)
		copy(bridgeID[2:], macBytes)
		a.mib.Set(dot1dStpDesignatedRoot, &OIDValue{
			Type:  gosnmp.OctetString,
			Value: bridgeID,
		})

		// dot1dStpRootCost
		a.mib.Set(dot1dStpRootCost, &OIDValue{
			Type:  gosnmp.Integer,
			Value: 0,
		})

		// dot1dStpRootPort
		a.mib.Set(dot1dStpRootPort, &OIDValue{
			Type:  gosnmp.Integer,
			Value: 0, // Root bridge has no root port
		})
		...
		a.mib.Set(dot1dStpHoldTime, &OIDValue{
			Type:  gosnmp.Integer,
			Value: 1, // 1 second
		})

		forwardDelay := stp.ForwardDelay
		if forwardDelay == 0 {
			forwardDelay = 15
		}
		...
		// Bridge-configured timers
		a.mib.Set(dot1dStpBridgeMaxAge, &OIDValue{
			Type:  gosnmp.Integer,
			Value: maxAge * 100,
		})
		a.mib.Set(dot1dStpBridgeHelloTime, &OIDValue{
			Type:  gosnmp.Integer,
			Value: helloTime * 100,
		})
		a.mib.Set(dot1dStpBridgeForwardDelay, &OIDValue{
			Type:  gosnmp.Integer,
			Value: forwardDelay * 100,
		})
		...
		for portIdx := 1; portIdx <= numPorts; portIdx++ {
			...
			// dot1dStpPortPriority (default 128)
			a.mib.Set(dot1dStpPortPriority+"."+portStr, &OIDValue{
				Type:  gosnmp.Integer,
				Value: 128,
			})

			// dot1dStpPortState (5 = forwarding)
			a.mib.Set(dot1dStpPortState+"."+portStr, &OIDValue{
				Type:  gosnmp.Integer,
				Value: 5,
			})

			// dot1dStpPortEnable (1 = enabled)
			a.mib.Set(dot1dStpPortEnable+"."+portStr, &OIDValue{
				Type:  gosnmp.Integer,
				Value: 1,
			})

			// dot1dStpPortPathCost
			a.mib.Set(dot1dStpPortPathCost+"."+portStr, &OIDValue{
				Type:  gosnmp.Integer,
				Value: 4, // Cost for 10G
			})

			// dot1dStpPortDesignatedRoot
			a.mib.Set(dot1dStpPortDesignatedRoot+"."+portStr, &OIDValue{
				Type:  gosnmp.OctetString,
				Value: bridgeID,
			})

			// dot1dStpPortDesignatedCost
			a.mib.Set(dot1dStpPortDesignatedCost+"."+portStr, &OIDValue{
				Type:  gosnmp.Integer,
				Value: 0,
			})

			// dot1dStpPortDesignatedBridge
			a.mib.Set(dot1dStpPortDesignatedBridge+"."+portStr, &OIDValue{
				Type:  gosnmp.OctetString,
				Value: bridgeID,
			})

			// dot1dStpPortDesignatedPort
			a.mib.Set(dot1dStpPortDesignatedPort+"."+portStr, &OIDValue{
				Type:  gosnmp.OctetString,
				Value: []byte{0x80, byte(portIdx)},
			})

			// dot1dStpPortForwardTransitions
			a.mib.Set(dot1dStpPortForwardTransitions+"."+portStr, &OIDValue{
				Type:  gosnmp.Counter32,
				Value: uint32(1),
			})
		}
	...
	// dot1dTp group - Transparent bridging
	a.mib.Set(dot1dTpLearnedEntryDiscards, &OIDValue{
		Type:  gosnmp.Counter32,
		Value: uint32(0),
	})

	a.mib.Set(dot1dTpAgingTime, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 300, // 300 seconds = 5 minutes
	})

	// dot1dTpFdbTable - Forwarding database entries
	// Add self entry for device MAC
	macHexStr := strings.ReplaceAll(device.MACAddress.String(), ":", "")
	macHexStr = strings.ReplaceAll(macHexStr, "-", "")
	// Convert MAC to index format (decimal octets separated by dots)
	macIndex := macToOIDIndex(device.MACAddress.String())

	a.mib.Set(dot1dTpFdbAddress+"."+macIndex, &OIDValue{
		Type:  gosnmp.OctetString,
		Value: macBytes,
	})
	a.mib.Set(dot1dTpFdbPort+"."+macIndex, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 0, // Self
	})
	a.mib.Set(dot1dTpFdbStatus+"."+macIndex, &OIDValue{
		Type:  gosnmp.Integer,
		Value: 4, // self
	})
	...
	for portIdx := 1; portIdx <= numPorts; portIdx++ {
		...
		// dot1dTpPort
		a.mib.Set(dot1dTpPort+"."+portStr, &OIDValue{
			Type:  gosnmp.Integer,
			Value: portIdx,
		})

		// dot1dTpPortMaxInfo
		a.mib.Set(dot1dTpPortMaxInfo+"."+portStr, &OIDValue{
			Type:  gosnmp.Integer,
			Value: 1500,
		})

		// dot1dTpPortInFrames (dynamic)
		...
		a.mib.Set(dot1dTpPortInDiscards+"."+portStr, &OIDValue{
			Type:  gosnmp.Counter32,
			Value: uint32(0),
		})
	}
...
func (a *Agent) initializeLLDPLocalMIB() {
	...
	a.mib.Set(lldpLocSysCapSupported, &OIDValue{
		Type:  gosnmp.OctetString,
		Value: []byte{byte(capabilities >> 8), byte(capabilities)},
	})
...
func getInterfaceSpeed(ifName string) uint64 {
	...
	case strings.Contains(ifNameLower, "ethernet"):
		return 1000000000 // Default to 1 Gbps
	default:
		return 1000000000 // Default to 1 Gbps
	}
}
