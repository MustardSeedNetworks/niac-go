...
| --- | --- | --- | --- | --- |
| Run simulation on a selected interface with a config file | `Niac <interface> <network.cfg>` | `niac daemon --once <iface> <config>` | `/runtime` interface picker + config picker | Covered |
| Set debug level 0-3 | `-d<n>` | `niac daemon --debug/-d <0-3>` | Protocol debug page and runtime/debug APIs | Covered |
| Run without interactive controls | default Java run | `niac daemon --once` | Start simulation from `/runtime` | Covered |
