...
func writeFixtureCert(t *testing.T) (string, string) {
	...
	template := x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "niac-install-ca-test"},
...
func TestInstallCACmdRegistered(t *testing.T) {
	...
	addInstallCACommand(root)

	cmd, _, err := root.Find([]string{"install-ca"})
	if err != nil {
...
func TestInstallCAHelpRuns(t *testing.T) {
	...
	root.SetArgs([]string{"install-ca", "--help"})
...
func TestResolveCertPath_Missing(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "niac daemon") {
		t.Errorf("expected hint about starting niac daemon, got: %v", err)
	}
}
...
func TestDefaultInstallCAPathEndsInCrt(t *testing.T) {
	got := defaultInstallCAPath()
	if got == "" {
		t.Error("defaultInstallCAPath returned empty string")
	}
	if !strings.HasSuffix(got, ".crt") {
		t.Errorf("defaultInstallCAPath should end in .crt, got %q", got)
	}
}
