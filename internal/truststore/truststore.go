...
//
// Lifted from seed; keep in sync. The implementation is intentionally small:
// it shells out to the platform utility that is already required to maintain
// the trust store on each supported OS (the same approach mkcert takes).
// Compared to depending on mkcert's internals — which are not exposed as a
// library API — this keeps niac in control of error reporting and avoids
// pulling in mkcert's command-line surface.
//
// Supported platforms:
//
...
// invoke `niac install-ca` under sudo rather than re-implementing
