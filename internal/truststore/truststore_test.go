...
// looks like the cert niac generates for HTTPS.
func makeFixtureCert(t *testing.T) string {
	...
	template := x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "niac-truststore-test"},
	...
	path := filepath.Join(t.TempDir(), "niac-test.crt")
...
func TestValidateCertFile_OK(t *testing.T) {
	...
	if cert.Subject.CommonName != "niac-truststore-test" {
