...
func assertEnterpriseDeviceMix(t *testing.T, cfg *config.Config) {
	...
	for _, site := range []string{"COS", "EVT", "EHV", "LON"} {
		...
		// A printer or a UPS fills an endpoint slot like any other wired
		// client, so it counts here — otherwise adding one to the mix reads as
		// the site having lost clients rather than gained a device.
		wiredClients := countNamed(cfg, site+"-WS-") + countNamed(cfg, site+"-LAP-") +
			countNamed(cfg, site+"-MBP-") + countNamed(cfg, site+"-PRN-") + countNamed(cfg, site+"-UPS-")
		if wiredClients != 64 {
