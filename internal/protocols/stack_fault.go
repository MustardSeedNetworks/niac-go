...
// ActiveInterfaceFaults returns a JSON-ready snapshot keyed by device address.
func (s *Stack) ActiveInterfaceFaults() map[string]map[string]map[devicestate.FaultType]int {
	...
	for device, store := range s.deviceStates {
		addActiveInterfaceFaults(result, deviceFaultAddress(device, store), store.Snapshot().Faults)
	}
	return result
}
...
func (s *Stack) interfaceFaultTarget(deviceIP string) (*config.Device, *devicestate.Store, error) {
	address, err := netip.ParseAddr(deviceIP)
	if err != nil {
...
func deviceFaultAddress(device *config.Device, store *devicestate.Store) string {
...
func (s *Stack) advanceDeviceFaultTelemetry(device *config.Device, now time.Time) {
	...
	group.telemetry.AdvanceInterfaceFaults(now, store.Snapshot().Faults, interfaceFaultSpeeds(device))
}
