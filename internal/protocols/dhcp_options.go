...
func (h *DHCPHandler) buildDHCPOptions(
	...
	options := []layers.DHCPOption{
		...
		{Type: layers.DHCPOptServerID, Length: dhcpIPv4Len, Data: serverIP},
...
// Caller must hold h.mu (at least RLock).
func (h *DHCPHandler) appendMiscOptions(options []layers.DHCPOption, clientMAC []byte) []layers.DHCPOption {
	...
	if lease, ok := h.leases[macStr]; ok && lease.Hostname != "" {
