...
// Carried over with LogMatchesFilter when internal/ipc was deleted: the
// function moved, so its coverage moves with it.
//
...
func TestFilterMatchesEveryFieldTheUserCanSee(t *testing.T) {
	...
	for _, filter := range []string{"med-acc-sw02", "STACK", "lldp", "LINK STATE"} {
		if !cliclient.LogMatchesFilter(entry, filter) {
			t.Errorf("filter %q did not match a record the user can see it in", filter)
		}
	}
	if cliclient.LogMatchesFilter(entry, "nothing-here") {
...
func TestEmptyFilterKeepsEverything(t *testing.T) {
	if !cliclient.LogMatchesFilter(cliclient.LogEntry{Message: "anything"}, "") {
...
func TestFilterFoldsCaseBeyondASCII(t *testing.T) {
	entry := cliclient.LogEntry{Message: "ÜBERTRAGUNG fehlgeschlagen"}

	if !cliclient.LogMatchesFilter(entry, "übertragung") {
