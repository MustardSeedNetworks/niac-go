...
// TestListFilesReportsWalkProvenance: the catalog's honesty column. A walk
// says whether its content was captured from a device or generated by NIAC;
// a pcap says nothing, and neither does a walk with no header.
func TestListFilesReportsWalkProvenance(t *testing.T) {
	lib := openTempLibrary(t)
	root := lib.Root()

	bodies := map[string]string{
		"walks/captured.walk":  "# Source: captured\n.1.3.6.1 = STRING: x\n",
		"walks/generated.walk": "# Source: generated\n.1.3.6.1 = STRING: y\n",
		"walks/silent.walk":    ".1.3.6.1 = STRING: z\n",
	}
	...
	if err := os.WriteFile(filepath.Join(root, "pcaps", "sample.pcap"), []byte("fake"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	want := map[string]walkmeta.Provenance{
		"captured.walk":  walkmeta.Captured,
		"generated.walk": walkmeta.Generated,
		"silent.walk":    walkmeta.Unknown,
	}
	for _, entry := range entries {
		if expected, known := want[entry.Name]; known && entry.Provenance != expected {
			t.Errorf("%s provenance = %q, want %q", entry.Name, entry.Provenance, expected)
		}
	}

	pcaps, err := lib.ListFiles(library.KindPcaps)
	if err != nil {
		t.Fatalf("list pcaps: %v", err)
	}
	for _, entry := range pcaps {
		if entry.Provenance != walkmeta.Unknown {
			t.Errorf("pcap %s carries provenance %q; only walks declare one", entry.Name, entry.Provenance)
		}
	}
}

// TestShippedStarterWalksDeclareTheirProvenance is the in-Go half of
// scripts/check-starter-walks.sh: over half the starter set is generated
// rather than captured, and the catalog must say so.
func TestShippedStarterWalksDeclareTheirProvenance(t *testing.T) {
	walks := os.DirFS(filepath.Join("starter", "walks"))
	names, err := fs.Glob(walks, "*.walk")
	if err != nil {
		t.Fatal(err)
	}
	if len(names) == 0 {
		t.Fatal("no starter walks found; this test would pass forever on a moved directory")
	}
	for _, name := range names {
		content, readErr := fs.ReadFile(walks, name)
		if readErr != nil {
			t.Fatalf("read %s: %v", name, readErr)
		}
		if walkmeta.Parse(content) == walkmeta.Unknown {
			t.Errorf("%s declares no provenance", name)
		}
	}
}
