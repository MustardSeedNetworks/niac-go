...
func TestServerHandleReplayRoutes(t *testing.T) {
	...
	// Use the same directory as the config so path validation passes.
	configDir := filepath.Dir(configPath)

	pcapPath := filepath.Join(configDir, "demo.pcap")
	err := os.WriteFile(pcapPath, []byte("pcap"), 0o600)
	if err != nil {
...
func TestServerHandleReplayUpload(t *testing.T) {
	...
	// PCAP header is 24 bytes minimum
	pcapData := []byte{
		...
		0x01, 0x00, 0x00, 0x00, // Link-layer header type (1 = Ethernet)
	}

	payload := map[string]string{
		"file": "uploaded.pcap",
		"data": base64.StdEncoding.EncodeToString(pcapData),
	}
...
func TestServerHandleFilesWalks(t *testing.T) {
	...
	err := os.WriteFile(walkPath, []byte("walk"), 0o600)
	if err != nil {
