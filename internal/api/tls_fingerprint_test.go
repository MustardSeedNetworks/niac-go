...
func TestFingerprintFromPEM(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("fingerprintFromPEM: unexpected error: %v", err)
	}
...
func TestFingerprintFromPEM_NoCertificateBlock(t *testing.T) {
	...
	if !errors.Is(err, errNoCertificateBlock) {
		t.Errorf("expected errNoCertificateBlock, got %v", err)
	}
}
...
func TestComputeCertFingerprint_EmptyPath(t *testing.T) {
	...
	_, err := computeCertFingerprint("")
	if !errors.Is(err, errEmptyCertPath) {
		t.Errorf("expected errEmptyCertPath, got %v", err)
	}
}
...
func TestComputeCertFingerprint_ReadsFile(t *testing.T) {
	...
	got, err := computeCertFingerprint(path)
	if err != nil {
		t.Fatalf("computeCertFingerprint: %v", err)
	}
	sum := sha256.Sum256(der)
	want := formatFingerprint(sum[:])
	if got != want {
