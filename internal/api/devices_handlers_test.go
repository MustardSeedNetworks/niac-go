...
func TestBuildTrafficConfigResponse(t *testing.T) {
	t.Run("nil config", func(t *testing.T) {
		dev := &config.Device{}
		resp := buildTrafficConfigResponse(dev)
		if resp != nil {
			t.Error("should return nil when no traffic config")
		}
	})

	t.Run("disabled", func(t *testing.T) {
		dev := &config.Device{
			TrafficConfig: &config.TrafficConfig{Enabled: false},
		}
		resp := buildTrafficConfigResponse(dev)
		if resp != nil {
			t.Error("should return nil when traffic disabled")
		}
	})
	...
	t.Run("enabled", func(t *testing.T) {
		...
		if !resp.ARPEnabled {
			t.Error("ARPEnabled should be true")
		}
		if !resp.PingEnabled {
			t.Error("PingEnabled should be true")
		}
	})
}
