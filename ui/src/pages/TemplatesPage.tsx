...
export const TemplatesPage: FC = () => {
	...
	const [showUseTemplateModal, setShowUseTemplateModal] =
		...
		async (configName: string) => {
			...
			try {
				...
				setMessage({
					...
					text:
						response.message ||
						`Configuration created at ${response.configPath}`,
				});
				...
				setMessage({
					...
					text:
						getErrorMessage(err) ||
						"Failed to create configuration from template",
				});
			}
		},
	...
	return (
		<div className="space-y-6">
					...
					<div className="flex flex-wrap items-center justify-between gap-4">
						<div>
							...
							<P className="text-gray-400 mt-1">
								Browse and use pre-configured network templates to quickly start
								simulations.
							</P>
						</div>
						<Button
			...
			{templates && templates.length > 0 && filteredTemplates.length === 0 && (
				<Card className="border-white/5 bg-gray-900/70">
					<CardContent className="py-12 text-center">
						...
						<P className="text-gray-400">
							No templates match your search &quot;{searchQuery}&quot;. Try a
			...
			<InputModal
				...
				defaultValue={
					showUseTemplateModal ? `${showUseTemplateModal.name}-config` : ""
				}
				submitLabel="Create"
				submitTone="violet"
			/>
