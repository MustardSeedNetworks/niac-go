...
export const DashboardPage: FC = () => {
	...
	return (
		<div className="space-y-6 animate-fade-in">
			{/* Status banner */}
			<Card
				className={`border-l-4 ${isRunning ? "border-l-emerald-500" : "border-l-gray-500"}`}
			>
						...
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p className="text-sm text-gray-400">
									{stats?.interface ?? "No interface"} •{" "}
									{stats?.deviceCount ?? 0} devices
								</p>
							</div>
						</div>
				...
				<Card hover={true} className="group">
					<CardContent className="space-y-1">
						...
						<p className="text-xs text-gray-500">
							{stats
								? `${formatNumber(stats.stack.packetsSent)} sent`
								: "Awaiting data"}
						</p>
					</CardContent>
				</Card>
	...
	({ errorTypes, info }: { errorTypes: ErrorType[]; info: string }) => (
		<div className="mt-4 rounded-xl border border-yellow-500/20 bg-yellow-900/10 p-4">
			...
			<div className="mt-3 rounded-lg bg-blue-900/20 p-3 text-sm text-blue-200">
				...
				to access interactive error injection with keyboard controls (press 'i'
				for menu, keys 1-7 for quick injection).
			</div>
		</div>
	),
);
...
const AutomationTimeline = memo(
	({ history }: { history: HistoryRecord[] | null }) => {
