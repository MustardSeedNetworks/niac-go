		...
		<div className="space-y-6">
			<Card className="border-white/5 bg-gray-900/70">
				<CardContent className="space-y-4">
					...
					<P>
						Define alert thresholds, webhook routes, and (soon) runnable
						workflow automations. Use the CLI flags today, then manage them
						graphically here as the 2.0 UI matures. Current alert counter:{" "}
						{stats?.stack.errors ?? 0}.
					</P>
					<ul className="space-y-3 text-sm text-gray-300">
						<li className="rounded-lg border border-white/5 bg-gray-950/50 p-3">
							Webhooks inherit settings from the `--alert-webhook` flag and can
							be overridden per run.
						</li>
						<li className="rounded-lg border border-white/5 bg-gray-950/50 p-3">
							Packet thresholds mirror CLI/TUI options so headless and web
							control stay aligned.
						</li>
						<li className="rounded-lg border border-white/5 bg-gray-950/50 p-3">
							Future: orchestrate multi-run scenarios and publish signed run
							reports.
						</li>
					</ul>
				</CardContent>
		...
		<Card className="border-white/5 bg-gray-900/70">
			<CardContent className="space-y-4">
				...
				<P className="text-gray-300">
					Updates take effect immediately—no CLI restart required. Leave the
					threshold blank or zero to disable packet alerts entirely.
				</P>
				{loading && (
