...
const DeviceNode: FC<{ data: DeviceNodeData; selected?: boolean }> = memo(
	({ data, selected }) => {
...
function layoutNodes(
	...
	for (const device of sorted) {
		if (ringIndex >= nodesInRing) {
			...
			nodesInRing = Math.max(
				1,
				Math.floor((2 * Math.PI * currentRadius) / 200),
			);
		}
...
const getLinkSpeed = (label?: string): string | undefined => {
	if (!label) {
		return;
	}
...
const getLinkType = (label?: string): LinkEdgeData["linkType"] | undefined => {
...
const getVlan = (label?: string): number | undefined => {
	if (!label) {
		return;
	}
...
const getEdgeColor = (data: LinkEdgeData): string => {
	...
	return links.map((link, index) => {
		...
		return {
			...
			style: {
				stroke: edgeColor,
				strokeWidth:
					data.linkType === "trunk" || data.linkType === "lag" ? 3 : 2,
			},
			markerEnd: {
...
const TopologyLegend: FC<{ show: boolean; onToggle: () => void }> = ({
	show,
	onToggle,
}) => {
	if (!show) {
...
export const TopologyPage: FC = () => {
	const { data: topology, loading: topologyLoading } = useApiResource(
		fetchTopology,
		[],
		{
			intervalMs: 15000,
	...
	const { data: devices, loading: devicesLoading } = useApiResource(
		fetchDevices,
		[],
		{
			intervalMs: 15000,
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<SmallText className="text-gray-400">
									{devices?.length || 0} devices | {topology?.links.length || 0}{" "}
									connections
								</SmallText>
							</div>
						</div>
						...
						<div className="absolute inset-0 flex items-center justify-center">
							<div className="text-center">
								...
								<SmallText className="text-gray-500">
									Configure devices with trunk ports or port-channels to
