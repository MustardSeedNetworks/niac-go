	...
	return (
		<Card className="border-white/5 bg-gray-900/70">
			<CardContent className="space-y-4">
				...
				{loading && (
					<SmallText className="text-gray-400">Loading devices...</SmallText>
				)}
				{error && (
				...
				<SmallText className="text-gray-400">
					Devices are rendered directly from the active YAML config so the
					CLI/TUI and Web UI always agree.
				</SmallText>
			</CardContent>
		</Card>
	);
	...
	return (
		<div className="rounded-xl border border-white/5">
			<div className="bg-gray-900/60 px-4 py-2 text-xs text-gray-400">
				Showing {virtualScroll.visibleItems.length} of {devices.length} devices
		...
		<td className="px-4 py-3">
			<div className="flex flex-wrap gap-2">
				...
				{device.protocols.length === 0 && (
					<SmallText className="text-gray-400">None</SmallText>
				)}
			</div>
		</td>
	</tr>
	...
	return (
		<Card className="border-white/5 bg-gray-900/70">
			<CardContent className="space-y-4">
				...
				{error && (
					<SmallText className="text-red-400">
						Unable to load config: {error.message}
					</SmallText>
				)}
				{data && (
					<>
						...
						<SmallText className="text-gray-400">
							Saving runs full validation (same as `niac validate`) before
							persisting so runtime changes stay safe.
						</SmallText>
