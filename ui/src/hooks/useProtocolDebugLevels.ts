...
export function useProtocolDebugLevels(): UseProtocolDebugLevelsResult {
	const [protocols, setProtocols] = useState<ProtocolDebugConfig[]>(
		DEFAULT_PROTOCOL_CONFIGS,
	);
	const [savedProtocols, setSavedProtocols] = useState<ProtocolDebugConfig[]>(
		DEFAULT_PROTOCOL_CONFIGS,
	);
	...
	const updateLevel = useCallback(
		(protocol: DebugProtocol, level: DebugLevel) => {
			setProtocols((prev) =>
				prev.map((p) => (p.protocol === protocol ? { ...p, level } : p)),
			);
			clearMessage();
		},
		[clearMessage],
