					...
					<div className="rounded-lg border border-white/5 bg-gray-950/40 p-4 space-y-3">
						...
						{device.traffic.arpAnnouncements?.enabled && (
							<FormField
								...
								helpText="Time between ARP announcements"
							>
								<input
									type="number"
						...
						{device.traffic.randomTraffic?.enabled && (
							<div className="space-y-4">
								<div className="grid gap-4 md:grid-cols-2">
									<FormField
										label="Interval (seconds)"
										helpText="Time between traffic bursts"
									>
										<input
											type="number"
								...
								<div className="space-y-2">
									...
									<div className="flex flex-wrap gap-4">
										{(["broadcast_arp", "multicast", "udp"] as const).map(
											(pattern) => (
												<label
													key={pattern}
												...
												>
													<input
														type="checkbox"
														checked={(
															device.traffic?.randomTraffic?.patterns || []
														).includes(pattern)}
														onChange={(e) => {
															const baseConfig = getTrafficConfig();
															const patterns =
																baseConfig.randomTraffic.patterns || [];
															if (e.target.checked) {
																updateTraffic({
																	...baseConfig,
																	randomTraffic: {
