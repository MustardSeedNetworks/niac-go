 ...
 */
const HexRow = memo(
	({ offset, bytes, bytesPerRow, headerLength, startIndex }: HexRowProps) => {
		...
		const padding =
			paddingCount > 0 ? (
				<span className="text-gray-700">{"   ".repeat(paddingCount)}</span>
			) : null;
		...
		const asciiChars = bytes.map((byte, idx) => {
			...
			return (
				<span
					key={globalIndex}
					className={isHeader ? "text-cyan-400" : "text-gray-300"}
				>
					{char}
				</span>
			);
		...
		return (
			<div className="flex font-mono text-xs leading-5">
				{/* Offset column */}
				<span className="text-violet-400 w-20 flex-shrink-0">
					{formatOffset(offset)}:
				</span>
