...
const toLogLevelKey = (level: LogLevel): LogLevelKey =>
	level.toLowerCase() as LogLevelKey;
...
const LogEntryRow: FC<{ log: LogEntry; searchQuery: string }> = memo(
	({ log, searchQuery }) => {
			...
			<div className={`border-b ${colors.border} transition-colors`}>
				...
				>
					...
					>
						{/* Expand indicator */}
						<span
							className={`shrink-0 mt-0.5 ${hasDetails ? "text-gray-500" : "text-transparent"}`}
						>
							{expanded ? (
						...
						{/* Level indicator bar */}
						<span
							className={`shrink-0 w-1 h-5 rounded-full ${colors.accent}`}
						/>
...
export const LogViewer: FC<LogViewerProps> = memo(
	({ logs, searchQuery, autoScroll }) => {
					...
					{Object.entries(stats).map(([level, count]) => {
						...
						return (
							...
							>
								...
								<span
									className={`${colors?.accent || "bg-gray-500"} h-2 w-2 rounded-full`}
								/>
								<span
									className={`font-medium ${colors?.text || "text-gray-400"}`}
								>
									{level}
								</span>
