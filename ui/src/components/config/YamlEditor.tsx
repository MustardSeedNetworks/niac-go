 ...
 */
export const YamlViewer: FC<Omit<YamlEditorProps, "readOnly" | "onChange">> = (
	props,
...
function validateYaml(content: string): string[] {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (line.includes("\t")) {
			errors.push(
				`Line ${lineNum}: Tabs are not allowed in YAML, use spaces instead`,
			);
		}
		...
		if (leadingSpaces > 0 && leadingSpaces % 2 !== 0) {
			errors.push(
				`Line ${lineNum}: Inconsistent indentation (should be multiple of 2 spaces)`,
			);
		}
