...
export const fetchNeighbors = () =>
	request<NeighborRecord[]>("/api/v1/neighbors");
...
export const fetchErrorTypes = () =>
	request<ErrorInjectionInfo>("/api/v1/errors");
...
export const fetchInterfaces = () =>
	request<InterfacesResponse>("/api/v1/interfaces");
...
export const updateProtocolDebugLevels = (
	payload: UpdateProtocolDebugLevelsRequest,
) =>
...
export const analyzePcap = (analysisId: string) =>
	request<PcapAnalysisResult>(
		`/api/v1/pcap/analyze/${encodeURIComponent(analysisId)}`,
	);
...
export const deleteDevice = (hostname: string) =>
	request<DeviceMutationResponse>(
		`/api/v1/config/devices/${encodeURIComponent(hostname)}`,
		{
			method: "DELETE",
		},
