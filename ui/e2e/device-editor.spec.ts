 ...
 * internal/api/devices_authored_contract_test.go and the Go handler tests;
 * treat this spec as UI wiring only.
 */
test.describe('Device Editor API wiring', () => {
  test('creates a device from the generated form as one authored document', async ({ page }) => {
    ...
    await page.route('**/api/v1/**', async (route) => {
      ...
      // The real path. It used to stub `/api/v1/files?kind=walks`, which no
      // client calls, so the walk list fell through to the catch-all `{}`.
      if (url.pathname === '/api/v1/library/walks') {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify([]),
      ...
      if (url.pathname.startsWith('/api/v1/device-schemas/')) {
        await route.fulfill({
          ...
          body: JSON.stringify({
            type: 'switch',
            label: 'Switch',
            visibleSections: ['basic', 'interfaces'],
          }),
        });
        return;
      }
    ...
    await page.locator('#device-mac').fill('02:00:00:00:10:01');

    // The generated form: no interface section was hand-written, so this also
    // proves the schema-driven renderer reaches a nested list of objects.
    await page.getByRole('button', { name: /interfaces section, collapsed/i }).click();
    await page.getByRole('button', { name: 'Add Interfaces' }).click();
    await page.locator('#device-interfaces---name').fill('Ethernet1/1');
    await page.locator('#device-interfaces---speed').fill('10000');
    await page.locator('#device-interfaces---admin_status').selectOption('up');
    await page.locator('#device-interfaces---description').fill('uplink to core');
    ...
    expect(createPayload?.rawYaml).toContain('name: edge-switch-01');
    expect(createPayload?.rawYaml).toContain('mac: 02:00:00:00:10:01');
    expect(createPayload?.rawYaml).toContain('admin_status: up');
    expect(createPayload?.rawYaml).toContain('description: uplink to core');
  });
});
