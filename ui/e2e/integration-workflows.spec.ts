...
test.describe('Integration Workflows', () => {
  ...
  test.describe('Device Lifecycle Workflow', () => {
    ...
    test('should complete full device lifecycle: create -> edit -> clone -> delete', async ({
      page,
    }) => {
      ...
      const _deviceInList = page.getByText(testDevice);
      ...
      if (await editButton.isVisible()) {
        ...
        const hostnameFieldEdit = page
          .locator('input[name="hostname"], input[name="name"]')
          .first();
  ...
  test.describe('Configuration Comparison Workflow', () => {
    test('should compare two configurations', async ({ page }) => {
      ...
      const targetSelect = page
        .locator('select[name*="target" i], select[name*="right" i]')
        .first();
      ...
      const _diffView = page.locator('[class*="diff"], [class*="compare"]');
      await expect(page.locator('body')).toBeVisible();
  ...
  test.describe('PCAP Analysis Workflow', () => {
    test('should analyze PCAP file', async ({ page }) => {
      ...
      const _packetList = page.locator('table, [class*="packet-list"]');
      await expect(page.locator('body')).toBeVisible();
  ...
  test.describe('Simulation with Traffic Workflow', () => {
    test('should run simulation with error injection', async ({ page }) => {
      ...
      const _runningIndicator = page.locator('[class*="running"], [class*="active"]');
      await expect(page.locator('body')).toBeVisible();
