...
# declare CAP_NET_RAW + CAP_NET_ADMIN and run as a non-root user (niac:niac).
# For local dev (`make build && ./niac daemon`) those capabilities have to
# be set on the binary OR you have to sudo. These targets handle that
...
#
# Note: niac daemon mode without NIAC_API_TOKEN is unauthenticated — anyone
# who can reach the listen port can drive the simulation. That's deliberate
# for dev convenience; production-installed units should configure a token.
# =============================================================================

.PHONY: dev-run dev-start dev-stop dev-status dev-restart

DEV_BINARY ?= ./niac
DEV_ARGS   ?= daemon --listen :8082 --storage /tmp/niac-dev.db
DEV_PID    ?= /tmp/niac.pid
DEV_LOG    ?= /tmp/niac.log
...
ifeq ($(DEV_UNAME),Linux)
	...
	@printf "  Web UI: http://localhost:8082\n"
